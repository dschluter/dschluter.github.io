...
Under the <b>random effects</b> (mixed-effects) model we assume that each study estimates a system-specific effect size. There is no "one true" effect size under this model, only a mean effect size. This is more realistic than the fixed effects model for most data in ecology and evolution. Even though each study of male bibs was carried out on the same species (house sparrow), there is nevertheless likely to be heterogeneity from population to population, year to year, and even researcher to researcher if study methods are not the same.
...
```{r eval = TRUE, fig.width = 6, fig.height = 5, message = FALSE}
library(metafor)
