...
This workshop demonstrates principles of data analysis (estimation and model selection) in a Bayesian framework. For probability and likelihood calculations consult the "Probability" tab at the R tips site.
...
Selecting among candidate models requires a criterion for evaluating and comparing models. We've already investigated AIC in a previous workshop. Here we demonstrate the superficially similar Bayesian information criterion (BIC). The approach has a tendency to pick a simpler model than does AIC, but this should not be the basis for preferring or shunning it. 
...
1. Using the hypergeometric distribution, calculate the log-likelihood of each of a range of possible values for the number of elephants in the Park. Note that the total number of elephants is <i>N</i> = <i>n</i> + <i>m</i>, and that <i>m</i> is known (<i>m</i> = 27). Note also that only integer values for <i>n</i> (and <i>N</i>) are allowed, and that <i>n</i> cannot be smaller than <i>k</i> - <i>X</i>, the observed number of unmarked individuals in the second sample. 

2. Plot the log-likelihood function. Find the value of <i>n</i> that maximizes the likelihood. Add <i>m</i> to this number to obtain the maximum likelihood estimate for population size <i>N</i>.*
...
4. Decide on a minimum and maximum possible value for <i>n</i>, the number of unmarked elephants in the Park (or <i>N = n + m</i>, the total number). The minimum <i>n</i> can be as small as 0 but the likelihood will be 0 for values smaller than <i>k - X</i>, so it is practical to set the smallest <i>n</i> to something greater or equal to <i>k - X</i>. For this first exercise don't set the maximum too high. We'll explore what happens later when we set the maximum to be a very large number.
...
7. Create a vector containing the prior probabilities for each of the possible values for <i>n</i> that you included in your vector in (5). If you are using a flat prior then the vector will be the same length as your <i>n</i> vector, and each element will be the same. Plot the prior probabilities against <i>n</i>. If all is OK at this stage then the plot should show a flat line. Also, confirm that the prior probabilities sum to 1.

8. Now apply Bayes Theorem. Using your two vectors from (6) and (7), calculate the posterior probability for each possible value of <i>n</i> (or of <i>N = n + m</i>) between your minimum and maximum values. After your calculation, confirm that all the posterior probabilities sum to 1. Plot the posterior probability against <i>n</i> (or <i>N = n + m</i>). Compare with the shape of the likelihood curve.

9. What is the most probable value of <i>N = n + m</i>, given the data? Compare this with your previous maximum likelihood estimate.*

10. Calculate the 95% credible interval for <i>n</i> (or <i>N = n + m</i>, the total population size).** The procedure for finding the lower and upper limits of the credible interval is a bit like that for likelihood. The idea is illustrated in the figure below. Starting from the highest point of the posterior probability, slide a horizontal line downward until you reach a point at which the corresponding values for the parameter (indicated below by the dashed vertical lines) bracket an area of 0.95 under the curve.

<img src="images/credible.jpg" alt="">

<br>
Try to think of a method to find the values for <i>n</i> (or <i>N = n + m)</i> that correspond to an area under the curve equal to 0.95. Trial and error might work. At the bottom of this section I describe a crude method that worked for me, but try to come up with a method yourself first.
...
<small>* I got 133
<br>** I got lower: 106, upper: 187, but you may get a different answer if you used a different prior</small>
...
# 5. Vector with possible values for N. This will determine the prior I'm using.
N <- 86:200
...
```{r eval = TRUE, message = FALSE, fig.width = 5, fig.height=4}
library(ggplot2)
...
ggplot(x, aes(x = species, y = co2flux)) + 
    geom_point(size = 3, col = "firebrick", alpha = 0.5) + 
    geom_smooth(method = "lm", linewidth = 1, se = FALSE) + 
    geom_line(aes(y = predictMM), linewidth = 1) +
    labs(x = "Number of species", y = expression(paste(CO[2], " flux"))) + 
    theme_classic()
...
ggplot(x, aes(x = species, y = co2flux)) + 
    ...
    geom_smooth(method = "lm", linewidth = 1, se = FALSE) + 
    geom_line(aes(y = predictMM), linewidth = 1) +
    geom_line(aes(y = predictPWR), linewidth = 1, col = "red") +
    labs(x = "Number of species", y = expression(paste(CO[2], " flux"))) + 
    theme_classic()
```
