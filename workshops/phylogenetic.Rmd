...
knitr::opts_chunk$set(echo = TRUE, fig.width = 5.5, fig.height=4.5, message = FALSE, warning = FALSE)
```

In this workshop will explore some of the tools available in R for analyzing species as data points. For help with R commands to carry out resampling, see the [phylogeny]{style="font-weight: bold;"} tab on the R tips help pages.

***

## Rockfish evolution

Rockfish (genus <i>Sebastes</i>) are a hugely diverse group of mainly Pacific Ocean fishes. There are over 100 recognized species inhabiting everything from the intertidal zone to waters over 2000 m deep. They have some of the highest longevities of any fish, with a maximum reported age of 205 years. Data on maximum body size (length), age (lifespan) and habitat depth of 56 species is provided [here in a .csv file](https://www.zoology.ubc.ca/~bio501/R/data/rockfish.csv). The data were gathered from FishBase, from Love (2002), and by Travis Ingram.

A phylogenetic tree of the 56 species is provided [in a text file here](https://www.zoology.ubc.ca/~bio501/R/data/rockfish.phy) (use "save link as" to download). The tree, from Hyde & Vetter (2007), is a consensus Bayesian tree based on 7 mitochondrial and 2 nuclear genes.
...
5.  Which of the traits show strong phylogenetic signal? First, use a visual method. Place dots at the tips of the tree with dot size reflecting the value for the trait. Is there a tendency for closely related species to be more similar in their trait values than more distantly related species?
...
<small> \* 1.19 when explanatory variable is log(maxlength.cm) and response variable is log(maxage.y) <br>\*\* 0.625 </small>
...
2.  A convenient way to think about GLS is that it carries out an ordinary linear model after transforming the data such that after transformation, the residuals are uncorrelated. The transformation is based on the phylogenetic correlation matrix. On the R tips pages for Phylogenetic comparison I show how one can carry out the transformation and then make a scatter plot of the transformed variables. This is a diagnostic tool to detect outliers, nonlinear relationships, and other violations of assumptions. Carry out this transformation and plot the transformed variables. Do you see any outliers?
...
# 2. Transform and plot
v <- vcv(rtree,  model = "Brownian", corr = TRUE)
z <- lm.gls(x = rockfish$log.maxlength, y = rockfish$log.maxage, v = v)
visreg(z$lm.fit, "x")
