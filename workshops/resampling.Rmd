...
Birds of the Caribbean islands of the Lesser Antilles are descended from rare immigrants that arrived long ago from larger islands and the nearby mainland. How long ago did these ancestors arrive? The data here are the approximate dates of immigration, in millions of years ago, of each of 37 bird species now present on the Lesser Antilles (Ricklefs and Bermingham 2001, Science 294: 1522-1524). The dates are approximate, being based on a molecular clock. They are calculated from the difference in mitochondrial DNA sequences between each of the species and its closest living relative on much larger islands or the mainland. The data can be downloaded <a href="https://www.zoology.ubc.ca/~bio501/R/data/antilles.csv">here</a>.
...
7. Most of the familiar estimators of population parameters, such as sample mean and variance, are unbiased, which means that the mean of its sampling distribution equals the parameter value being estimated. For example, the mean of the sampling distribution of the sample mean $\bar X$ is $\mu$, the population mean, which is the very parameter being estimated when we calculate a sample mean. The sample mean is therefore $\bar X$ an unbiased estimator. However, some estimators are biased, and the bootstrap is often used to estimate bias. Is the median of immigration dates biased? Calculate the <b>mean</b> of the bootstrap replicate estimates of the median immigration date to estimate the bias**.
...
<small>* I log-transformed both variables
