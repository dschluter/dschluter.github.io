...
```{r eval = TRUE, fig.width = 5, fig.height=4, message=FALSE}
library(ggplot2)
library(visreg)
library(MASS)
...
```{r eval = TRUE, fig.width = 5.5, fig.height=5}
...
anova(z, test = "Chi")
visreg(z, xvar = "tarsus", by = "year", scale = "response", rug = FALSE)
...
6. Extract the estimated regression coefficients from your model object. What is the interpretation of these coefficients? 
...
14. Redo the test of significance for the slope of the relationship between number of satellite mates and female carapace width. Remember to use the $F$ test rather than the likelihood ratio test in the <code>anova()</code> command when using quasi-likelihood. How do the results compare with those from the previous fit?
