...
An example using R Markdown is found <a href="https://www.zoology.ubc.ca/~schluter/R/#Use_R_Markdown">here</a>. Save example files to a folder on your hard disk and then open with RStudio. To render it, click "Knit" at the top of the Source file pane in RStudio. 
...
* Include at least one categorical fixed factor as an explanatory variable, such as an experimental or observational treatment.
...
<!-- ################### ASSIGNMENT 3 - Agrostis ########################################### 
     The following data set was analyzed in Sept 2016, Sept 2018, Sept 2020, Jan 2024, Jan 2026 semesters -->
...
Height of plant tillers of different cross generations can be downloaded <a href="https://www.zoology.ubc.ca/~bio501/R/data/arsenic.csv">here</a>.
...
* Add a numeric variable in the data set to represent the proportion of the genome inherited from the high-tolerance parent:
<br>&nbsp; 1 for the high-tolerance parent genotype
<br>&nbsp; 0 for the low-tolerance parent genotype
<br>&nbsp; 0.5 for the F1 and F2 hybrids
<br>&nbsp; 0.25 for the backcross to the low tolerance population
<br>&nbsp; 0.75 for the backcross to the high tolerance population
<br>Make sure that the variable is numeric rather than a factor or character.
* Fit the numeric variable you created in (3) to the height data using a linear model. This is called the additive model, whereby tolerance increases linearly with the proportion of the genome inherited from the high tolerance parent. Evaluate the model fit (Remember: no <i>P</i> values!).
* Add another numeric variable to the data set to represent dominance effects that might be present in the hybrids:
<br>&nbsp; 0 for both parent genotypes
<br>&nbsp; 1 for the F1 hybrid
...
<!-- This data set was analyzed in Sept 2014, Sept 2017, Sept 2019, Jan 2022, Jan 2025 semesters -->
...
Clues to the genetic basis of species differences can be gained by fitting linear models to measurements of traits in parents and hybrids. In this assignment you will use model selection methods to compare the fit of alternative genetic models to oviposition preference data in two host races of the planthopper <i>Nilaparvate nugens</i> (Sezer and Butlin 1998, Proc. Roy. Soc. Lond. B 265: 2399-2405). One race occurs on cultivated rice. The other lives on the aquatic plant <i>Leersia</i>, which is probably the ancestral host.
...
2. Create a table of means and standard deviations of the genotypes. Make this a presentation quality table rather than simply computer output. Don't worry about font.
...
9. Explain how the procedure you used above to analyze these data differs from that of conventional null hypothesis significance testing. In your view, would a null hypothesis significance testing approach be a poorer, equivalent, or superior approach to the one used above to decide between the three models? Explain.
10. If you used an AI, include an acknowledgement at the end that explains how you used it.
11. Include your clean R code in an appendix.
